...
describe('UserService', () => {
  ...
  describe('Update User', () => {
    it('should successfully update a user', async () => {
      ...
      userRepository.save = jest.fn().mockResolvedValue({ ...existingUser, ...updateDto });

      const result = await userService.update('1', updateDto, currentUser);
    ...
    it('should throw UnauthorizedException if user is not authenticated', async () => {
      await expect(userService.update('1', { email: 'new@example.com' }, null)).rejects.toThrow(
    ...
    it('should throw NotFoundException if user does not exist', async () => {
      ...
      await expect(userService.update('1', { email: 'new@example.com' }, { user_type: 'admin' })).rejects.toThrow(
